...
Polygon andrewScan(Polygon s){
    ...
    {
        for(int n = u.size(); 
            n >= 2 && ccw(u[n-2],u[n-1],s[i])!=-1 && ccw(u[n-2],u[n-1],s[i])!=-2 ; n--)
        {
            u.pop_back();
        }
        u.push_back(s[i]);
    ...
    {
        for(int n = l.size();
            n >= 2 && ccw(l[n-2],l[n-1],s[i])!=-1 && ccw(l[n-2],l[n-1],s[i])!=-2; n--)
        {
            l.pop_back();
        }
        l.push_back(s[i]);
...
public:
    Point p;
    int seg;    // segment id    
    int st;     // Bottom:0 Left:1 Right:2 Top:3
    EndPoint() {};
...
int manhattanIntersection(vector<Segment> s){
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        if(s[i].p1.y==s[i].p2.y){
            EP[i*2]=EndPoint(s[i].p1,i,1);
            EP[i*2+1]=EndPoint(s[i].p2,i,2);
        }
        else{
            EP[i*2]=EndPoint(s[i].p1,i,0);
            EP[i*2+1]=EndPoint(s[i].p2,i,3);
        }
    }
    ...
    for(int i = 0; i < 2*n; i++)
    {
        //cout<<i<<"\tseg:"<<EP[i].seg<<"\tst:"<<EP[i].st<<"\tx:"<<EP[i].p.x<<"\ty:"<<EP[i].p.y<<endl;//
        if(EP[i].st==0)//bottom
        {
            BT.insert(EP[i].p.x);
        }
        else if(EP[i].st==3)//top
        {
            BT.erase(EP[i].p.x);
        }
        else if(EP[i].st==1)//left
        {
            auto lb = lower_bound(BT.begin(),BT.end(), s[EP[i].seg].p1.x);
            auto ub = upper_bound(BT.begin(),BT.end(), s[EP[i].seg].p2.x);
            cnt+=distance(lb, ub);
            //cout<<i<<" "<<EP[i].st<<" "<<s[EP[i].seg].p1.x<<" "<<distance(lb,ub)<<endl;//
